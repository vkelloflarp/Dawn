...
bool create_character(CharacterRace race,
    ...
    created = *source;
    created.soid = soid;
    // A created character is selected as it is created, and the player goes straight into the game
    // with it, as the retail flow does. The Client leaves its sign-in step when the account names a
    // selection. Whatever was selected before stays a character of the account but is released, so
    // one character at most holds the selection.
    ...
    // Two characters made from one template must not share item instances. The template's keys are
    // cleared first so none of them can block or collide with the fresh allocation below.
    ...
    // A new character starts the New Light introduction, which begins without weapons or travel
    // gear. That is the state the account load gives any character that has not started it, so a
    // character created now must not wait for the next load to be in it.
